...
const Accessories = () => {
  ...
  const [AccessoriesProduct, setAccessories] = useState([]);
  ...
  useEffect(() => {
    // Filter products by category 'Accessories'
    if (allProducts.length > 0) {
      const filteredProducts = allProducts.filter(
        (product) => product.category === "Accessories" // Only Accessories category 
      );
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      {AccessoriesProduct.length === 0 ? (
        <p className="text-center text-gray-700 font-medium">No Accessories  available.</p>
      ) : (
