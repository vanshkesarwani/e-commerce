...
const Beauty = () => {
  ...
  useEffect(() => {
    // Filter products by category 'Beauty'
    if (allProducts.length > 0) {
      const filteredProducts = allProducts.filter(
        (product) => product.category === "Beauty" // Only Beauty category
      );
      setBeautyProducts(filteredProducts);
    }
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      {beautyProducts.length === 0 ? (
        <p className="text-center text-gray-700 font-medium">No Beauty Products available.</p>
      ) : (
