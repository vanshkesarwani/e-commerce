...
// ==========================================
// LUXURY SEARCH RESULTS CATALOG & FILTER VIEW
// ==========================================
function SearchResults() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        setMatchedCategories(Array.isArray(data?.matchedCategories) ? data.matchedCategories : []);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [selectedCategory, sortBy, allResults]);
  ...
  return (
    <div ref={resultsTopRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {/* Header & Controls Bar */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mt-1 flex flex-wrap items-center gap-2">
            <span>Results for</span>
            <span className="text-indigo-600 bg-indigo-50 px-3 py-0.5 rounded-xl border border-indigo-200 font-black">
          ...
          <p className="text-sm text-slate-500 mt-2">
            {loading
              ? "Discovering matching catalog items..."
              : `Found ${filteredResults.length} related pieces across collections`}
          </p>

          {/* Matched Categories Badges */}
          {matchedCategories.length > 0 && (
            <div className="flex items-center gap-1.5 mt-3 flex-wrap">
              <span className="text-xs text-slate-400 font-medium">Related Categories:</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-200/80 p-8 sm:p-12 mt-8 shadow-xs">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center text-2xl mb-4">
            <FaSearch />
          </div>
          <h3 className="text-2xl font-extrabold text-slate-900">
            No exact matches for "{keyword}"
          </h3>
          <p className="text-sm text-slate-500 max-w-md mx-auto mt-2 leading-relaxed">
            We couldn't find any products matching your query. Check for typos or try one of our popular search terms below:
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-6 max-w-lg mx-auto">
            {SUGGESTED_SEARCHES.map((term) => (
              <button
                ...
                className="px-3.5 py-1.5 rounded-full bg-slate-100 hover:bg-slate-900 hover:text-white text-xs font-semibold text-slate-700 transition"
              >
                {term}
              </button>
            ))}
          ...
          <div className="mt-8 flex justify-center gap-3">
            <Link
              to="/allproducts"
              className="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-bold uppercase tracking-wider hover:bg-slate-800 transition shadow-md"
            >
              Browse Complete Catalog (210 items)
            </Link>
          </div>
        </div>
      ) : (
        /* Classy Product Grid: 2 columns on mobile, 3 on tablet, 4 on desktop */
              ...
              <div className="relative w-full aspect-square bg-slate-100 overflow-hidden">
                ...
                <div className="absolute top-2 sm:top-3 left-2 sm:left-3 flex flex-col gap-1 z-10">
                  <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-slate-900/90 backdrop-blur-md text-white text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider shadow">
                    {product.category || "Velura"}
                  </span>
                </div>

                {/* Stock Status Badge */}
                {product.stock !== undefined && product.stock <= 5 && (
                  <span className="absolute top-2 sm:top-3 right-2 sm:right-3 px-1.5 sm:px-2 py-0.5 rounded-full bg-amber-500/90 backdrop-blur-md text-slate-900 text-[9px] sm:text-[10px] font-bold shadow">
                    {product.stock} left
                  </span>
                )}
              </div>
      ...
      {/* Modern Luxury Pagination & Discovery Bar for Search Results */}
          ...
          <div className="flex flex-col items-center space-y-2">
            <div className="text-xs font-semibold tracking-wider text-slate-500 uppercase flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="text-slate-900 font-extrabold bg-slate-100 px-2.5 py-0.5 rounded-full border border-slate-200">
                {indexOfFirstProduct + 1} – {Math.min(indexOfLastProduct, filteredResults.length)}
              </span>
              <span>of</span>
              <span className="text-slate-900 font-extrabold">{filteredResults.length}</span>
              <span>matches</span>
            </div>
            ...
            <div className="w-56 sm:w-72 h-1.5 bg-slate-100 rounded-full overflow-hidden p-0.5 border border-slate-200/60">
              <div
                ...
                style={{
                  width: `${Math.min(
                    100,
                    (Math.min(indexOfLastProduct, filteredResults.length) /
